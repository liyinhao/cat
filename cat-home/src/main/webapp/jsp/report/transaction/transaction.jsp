			...
			<c:forEach var="item" items="${model.displayNameReport.results}" varStatus="status">
				...
				<tr class=" right">
					...
					<c:choose>
						...
						<c:otherwise>
							<td><a href="/cat/r/m/${e.failMessageUrl}?domain=${model.domain}">${w:format(e.failCount,'#,###,###,###,##0')}</a></td>
						</c:otherwise>
					</c:choose>
					<td>&nbsp;${w:format(e.failPercent/100,'0.0000%')}</td>
					<td class="center"><a href="/cat/r/m/${empty e.successMessageUrl ? e.failMessageUrl : e.successMessageUrl}?domain=${model.domain}">Log View</a></td>
